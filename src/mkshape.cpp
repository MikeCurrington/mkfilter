...
static void trunc_coeffs()
{
	if (options & opt_b)
	{
		...
		int h = (numcoeffs-1) / 2;
		/* max coeff */
		double max = (options & opt_h) ? xcoeffs[h-1] : xcoeffs[h];
		double scale = (fac-1.0) / (fac*max);
		for (int i = 0; i < numcoeffs; i++)
		{	/* scale coeffs so max is (fac-1.0)/fac */
			double x = xcoeffs[i] * scale;
			/* truncate */
			xcoeffs[i] = fix(x*fac) / fac;
		}
	}
}
