...
static void printresults(const char* const []), printcmdline(const char* const []), printfilter(), printgain(const char*, complex);
...
global int main(int argc, const char * const argv[])
{
	...
	if (options & opt_re)
	{	/* bandpass resonator */
		if (options & opt_bp) compute_bpres();
		/* bandstop resonator (notch) */
		if (options & opt_bs) compute_notch();
		/* allpass resonator */
		if (options & opt_ap) compute_apres();
	}
	else
	{
		if (options & opt_pi)
...
static void normalize()
{
	...
	switch (options & (opt_lp | opt_hp | opt_bp| opt_bs))
	{
		...
		case opt_hp:
		{
			...
			/* also N zeros at (0,0) */
			for (i=0; i < splane.numpoles; i++) splane.zeros[i] = 0.0;
...
static void add_extra_zero()
{
	...
	/* ensure causality */
	while (zplane.numpoles < zplane.numzeros) zplane.poles[zplane.numpoles++] = 0.0;
}
